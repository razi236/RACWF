...
'm_acquire'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp505567264' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_acquireInstance',[O,[]],#task_info{method= <<"acquireInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
             %% abs/lang/abslang.abs:1153--1153
            future:await(maps:get('tmp505567264', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% abs/lang/abslang.abs:1153--1153
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp505567264', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
...
'm_release'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp145664926' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_releaseInstance',[O,[]],#task_info{method= <<"releaseInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
             %% abs/lang/abslang.abs:1160--1160
            future:await(maps:get('tmp145664926', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% abs/lang/abslang.abs:1160--1160
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp145664926', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
