...
'm_acquire'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp665448485' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_acquireInstance',[O,[]],#task_info{method= <<"acquireInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
             %% RPL/lang/RPLlang.RPL:1153--1153
            future:await(maps:get('tmp665448485', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% RPL/lang/RPLlang.RPL:1153--1153
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp665448485', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
...
'm_release'(O=#object{oid=Oid,cog=Cog=#cog{ref=CogRef,dcobj=DC}},Stack)->
    ...
    try
        ...
        case (not cmp:eq(C:get_val_internal(get(this), 'cloudprovider'),null)) of
            ...
            put(vars, (get(vars))#{'tmp1905486482' => cog:create_task(C:get_val_internal(get(this), 'cloudprovider'),'m_releaseInstance',[O,[]],#task_info{method= <<"releaseInstance"/utf8>>, creation={dataTime,builtin:currentms(Cog)}, proc_deadline=dataInfDuration},Cog)}),
             %% RPL/lang/RPLlang.RPL:1160--1160
            future:await(maps:get('tmp1905486482', get(vars)), Cog, [O,DC| Stack]),
            ok,
             %% RPL/lang/RPLlang.RPL:1160--1160
            put(vars, (get(vars))#{'result' := future:get_blocking(maps:get('tmp1905486482', get(vars)), Cog, [O,DC| Stack])});
            false ->         ok
        end,
