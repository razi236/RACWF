    ...
    try
        ...
        try
            ...
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('fut', get(vars)),[O,DC| Stack])) of
                ...
                put(vars, (get(vars))#{'tmp1777112002' => maps:get('fut', get(vars))}),
                 %% ABS.rpl:110--113
                []=(fun Loop ([])->
                    case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1777112002', get(vars)),[O,DC| Stack])) of
                    false -> [];
                    true -> receive
                            {stop_world, CogRef} ->
                         ...
                         %% ABS.rpl:110--113
                        put(vars, (get(vars))#{'f' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1777112002', get(vars)),[O,DC| Stack])}),
                         %% ABS.rpl:110--113
                        put(vars, (get(vars))#{'tmp1777112002' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1777112002', get(vars)),[O,DC| Stack])}),
                         %% ABS.rpl:112--112
