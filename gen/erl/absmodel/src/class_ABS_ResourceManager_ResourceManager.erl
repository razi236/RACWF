 ...
 'newResources' => V_newResources_0 }),
    try
        ...
        put(vars, (get(vars))#{'result' => m_ABS_StdLib_funs:f_list(Cog,[],[O,DC| Stack])}),
         %% ABS.rpl:28--32
        put(vars, (get(vars))#{'tmp1639341396' => maps:get('newResources', get(vars))}),
         %% ABS.rpl:28--32
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp1639341396', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                 ...
                 %% ABS.rpl:28--32
                put(vars, (get(vars))#{'r' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp1639341396', get(vars)),[O,DC| Stack])}),
                 %% ABS.rpl:28--32
                put(vars, (get(vars))#{'tmp1639341396' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp1639341396', get(vars)),[O,DC| Stack])}),
                 %% ABS.rpl:29--29
    ...
    try
        ...
        []=(fun Loop ([])->
            ...
            true -> receive
                 ...
                 %% ABS.rpl:41--53
                put(vars, (get(vars))#{'tmp421293587' => maps:get('wantedResources', get(vars))}),
                 %% ABS.rpl:41--53
                []=(fun Loop ([])->
                    case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp421293587', get(vars)),[O,DC| Stack])) of
                    false -> [];
                    true -> receive
                            {stop_world, CogRef} ->
                                cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                         ...
                         %% ABS.rpl:41--53
                        put(vars, (get(vars))#{'q' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp421293587', get(vars)),[O,DC| Stack])}),
                         %% ABS.rpl:41--53
                        put(vars, (get(vars))#{'tmp421293587' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp421293587', get(vars)),[O,DC| Stack])}),
                         %% ABS.rpl:42--42
                        put(vars, (get(vars))#{'found' => false}),
                         %% ABS.rpl:43--52
                        put(vars, (get(vars))#{'tmp967532279' => m_ABS_StdLib_funs:f_entries(Cog,maps:get('lr', get(vars)),[O,DC| Stack])}),
                         %% ABS.rpl:43--52
                        []=(fun Loop ([])->
                            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp967532279', get(vars)),[O,DC| Stack])) of
                            false -> [];
                            true -> receive
                                    {stop_world, CogRef} ->
                                        cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                                 ...
                                 %% ABS.rpl:43--52
                                put(vars, (get(vars))#{'entry' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp967532279', get(vars)),[O,DC| Stack])}),
                                 %% ABS.rpl:43--52
                                put(vars, (get(vars))#{'tmp967532279' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp967532279', get(vars)),[O,DC| Stack])}),
                                 %% ABS.rpl:44--46
 ...
 'ids' => V_ids_0 }),
    try
        ...
        put(vars, (get(vars))#{'all_ids' => m_ABS_StdLib_funs:f_keys(Cog,C:get_val_internal(get(this), 'resources'),[O,DC| Stack])}),
         %% ABS.rpl:78--83
        put(vars, (get(vars))#{'tmp5338884' => maps:get('ids', get(vars))}),
         %% ABS.rpl:78--83
        []=(fun Loop ([])->
            case not (m_ABS_StdLib_funs:f_isEmpty(Cog,maps:get('tmp5338884', get(vars)),[O,DC| Stack])) of
            false -> [];
            true -> receive
                    {stop_world, CogRef} ->
                        cog:task_is_blocked_for_gc(Cog, self(), get(task_info), get(this)),
                 ...
                 %% ABS.rpl:78--83
                put(vars, (get(vars))#{'id' => m_ABS_StdLib_funs:f_head(Cog,maps:get('tmp5338884', get(vars)),[O,DC| Stack])}),
                 %% ABS.rpl:78--83
                put(vars, (get(vars))#{'tmp5338884' := m_ABS_StdLib_funs:f_tail(Cog,maps:get('tmp5338884', get(vars)),[O,DC| Stack])}),
                 %% ABS.rpl:79--79
